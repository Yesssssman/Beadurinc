...
namespace StateGameplayTags
{
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT( State_JumpingLocked,			"State.JumpingLocked",				"A state where a player's jumping ability is blocked"						 );
	UE_DEFINE_GAMEPLAY_TAG_COMMENT( State_Invincible,				"State.Invincible",					"A state where invincible frame applied by dodge ability"					 );
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT( State_StunResist,				"State.StunResist",					"A state to protect a character from hit-stuns."							 );

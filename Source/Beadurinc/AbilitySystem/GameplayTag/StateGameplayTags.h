#pragma once

#include "NativeGameplayTags.h"

namespace StateGameplayTags
{
	/** GameplayTag definition macro */
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_ComboLocked				 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_Attacking				 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_StunResist				 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_Blocking				 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_BlockingLocked			 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_RollingLocked			 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_JumpingLocked			 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_Invincible				 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_Parry					 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_Stunned					 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_StaminaRegenCooldown	 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_LockOnRotationSuppressed );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_HoldComboSequence		 );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_VulnerableToExecution    );
	BEADURINC_API UE_DECLARE_GAMEPLAY_TAG_EXTERN( State_LowAttackDodgeable		 );
}

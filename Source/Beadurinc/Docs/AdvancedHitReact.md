...
언리얼에서 제공하는 타이머 함수를 통해 주어진 시간동안 `CustomTimeDilation` 을 0.0으로 설정하였다. `CustomTimeDilation` 은 특정
...
언리얼 엔진에서 자체적으로 지원하는 CameraShakeLegacy Blueprint를 사용하여 CameraShake를 구현하였다. 이 효과는 PlayerController
마다 적용되므로 HitReact가 발동한 플레이어 하나만을 콕 집어서 Camera Shake를 재생할 수 있는 장점이 있다.
